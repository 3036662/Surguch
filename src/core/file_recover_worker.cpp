...
void FileRecoverWorker::recoverFileWithByteRange(const QString path,
    ...
    if (!src_file.exists() || branges.size()!=2
            || eof_pos >=src_file.size()){
        qWarning()<<"[recoverFileWithByteRange] invalid parameters";
        emit recoverCompleted("");
        return;
    }
    // create temporary
    const QString tpm_dir=QStandardPaths::writableLocation(QStandardPaths::TempLocation)+"/";
    ...
    if (!dest_file.open(QIODevice::WriteOnly) || !src_file.open(QIODevice::ReadOnly)){
        dest_file.close();
        src_file.close();
