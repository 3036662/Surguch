...
PdfPageRender::PdfPageRender() { setFlag(QQuickItem::ItemHasContents, true);
                               ...
                               qWarning() << "Current Thread ID:" <<QThread::currentThread()->currentThreadId();
                               }
...
void PdfPageRender::geometryChange(const QRectF &newGeometry, const QRectF &oldGeometry){

     bool needs_new_rendering=false;
     if (oldGeometry!=newGeometry && pwidth_>1 && zoom_dpi_last_>0){
     float new_zoom_dpi_=width()/(pwidth_)*2;
...
QSGNode *PdfPageRender::updatePaintNode(
  ...
  if (fzctx_ != nullptr && fzdoc_ != nullptr && image_.isNull()) {
    ...
    float zoom_dpi=2;  //72 dpi x zoom
    //choose a zoom factor
    pwidth_=page_rect.x1-page_rect.x0;
    if (pwidth_>1){
        zoom_dpi=width()/(page_rect.x1-page_rect.x0)*2;
        qWarning()<<"zoom "<<zoom_dpi;
    }
    int page_width=(page_rect.x1 - page_rect.x0)*zoom_dpi;
    int page_height=(page_rect.y1 - page_rect.y0)*zoom_dpi;
    qWarning()  << " page width= "<<page_width<<"page_height "<< page_height;
    ...
    qWarning()  << " element width= "<<width()<<"element_height "<< height();

    pixmap = fz_new_pixmap(fzctx_, color_space, page_width,
                                      page_height, separation, 0);
    fz_clear_pixmap(fzctx_, pixmap);
    qWarning()  << " pixmap width= "<<page_width<<"pixmax height "<< page_height;



    float scale_factor =page_width>0 ? width() / page_width : 1;
    float scale_factor_y=page_height>0 ? height()/ page_height : 1;

    qWarning()  << " scale_factor = "<<scale_factor<<"scale_factor_y"<< scale_factor_y;

    if (scale_factor>100 || scale_factor_y>100 || scale_factor==0 || scale_factor_y==0){
    ...
    fz_matrix transform_device=fz_scale(scale_factor,scale_factor_y);
    draw_device = fz_new_draw_device(fzctx_, fz_identity, pixmap);
    ...
    auto buff_size = pix_stride * page_height;
    qWarning()<<"buff size "<<buff_size;


    buff_.clear();
    buff_.reserve(buff_size+1);
    std::copy(samples, samples + buff_size + 1, buff_.data());
    image_.reset(new QImage(buff_.data(), page_width, page_height, pix_stride,
                   QImage::Format_RGB888));
    zoom_dpi_last_=zoom_dpi;
    }
    fz_always(fzctx_){
    // clean up
       // fz_drop_pixmap(fzctx_, pix); // EXPERIMENTAL
