...
class TreeItem {
   public:
    ...
    /// @brief return data about node
    [[nodiscard]] const FileData &data() const;
    /// @brief checks if ancestors contain node with requested string
    [[nodiscard]] bool contains(const QString &full_path) const;

    /// @brief set hint and icon for signature column
    void setSigStats(const QString &text, const QString &color);
    /// @brief set hint and icon for mrpa column
