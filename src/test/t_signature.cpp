...
void TSignature::parseHexString() {
    ...
    QVERIFY(core::utils::hexStringToByteArray("0001020304050", 13) ==
            expected2);
    QVERIFY(core::utils::hexStringToByteArray("<000102030405>", 14) ==
            expected1);
    QVERIFY(core::utils::hexStringToByteArray("<0001020304050>", 15) ==
            expected2);
    QVERIFY(core::utils::hexStringToByteArray("<0001020304x050>", 16) ==
            expected_fail);
    QVERIFY(core::utils::hexStringToByteArray("<0001020304050", 14) ==
            expected2);
    QVERIFY(core::utils::hexStringToByteArray("0001020304050>", 14) ==
            expected2);
}
...
void TSignature::OneSigCoverage() {
    ...
    // gap at the beginnig
    {
        ...
        QVERIFY(info.coverage ==
                file_size -
                    info.sig_data_size);  // coverage is equal the signature
                                          // size,gives nothing
        QVERIFY(!sig_data.empty());
        QVERIFY(info.gaps_size == file_size - info.coverage);
        QVERIFY(info.invalid_range == false);
